...
async def analyze_competitor(payload: CompetitorAnalysisRequest, db: Session = Depends(get_db)) -> AnalysisAccepted:
    return await queue_analysis(db=db, user_id=payload.user_id, analysis_type="competitor", input_data=payload.model_dump())
